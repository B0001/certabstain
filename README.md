...
The composition is checked at construction — by `TwoSidedClaim.__post_init__`,
so the check runs whether an instance comes from `TwoSidedClaim.compose` or
is built directly. If the calibrated threshold does not clear the floor,
construction raises rather than returning a certificate that asserts a bound
it cannot support.
...
certabstain/
  ...
  test_*.py                 153 tests: theorem validation (Monte Carlo +
